...
def plot_in_grid(arr, out_path, cols=4, title=None):
    arr = np.asarray(arr)

    # If input is (B, T, D) with B=1, squeeze batch; if (T,) -> (T,1)
    if arr.ndim == 3 and arr.shape[0] == 1:
        arr = arr[0]
    if arr.ndim == 1:
        arr = arr.reshape(-1, 1)
    elif arr.ndim > 2:
        # Fallback: flatten trailing dims to D
        arr = arr.reshape(arr.shape[0], -1)

    T, D = arr.shape
    rows = (D + cols - 1) // cols

    # Always return a 2-D axes array
    fig, axes = plt.subplots(rows, cols, figsize=(3*cols, 2.2*rows), squeeze=False)

    for i in range(rows * cols):
        ax = axes[i // cols, i % cols]
        if i < D:
            ax.plot(np.arange(T), arr[:, i], alpha=0.8)
