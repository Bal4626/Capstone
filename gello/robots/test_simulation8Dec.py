    ...
    def serve(self) -> None:
        ...
        with mujoco.viewer.launch_passive(self._model, self._data) as viewer:
            while viewer.is_running():
                ...
                # ===== JACOBIAN-BASED FORCE-TO-TORQUE TRANSFORMATION =====
                if self._test_jacobian and self._ee_site_id >= 0:
                    # Only consider first 6 joints (UR5e arm)
                    nv_arm = 6
                    jac_pos = np.zeros((3, self._model.nv))
                    jac_rot = np.zeros((3, self._model.nv))
                    
                    # Compute full Jacobian at attachment site
                    mujoco.mj_jacSite(self._model, self._data, jac_pos, jac_rot, self._ee_site_id)
                    
                    # Assemble 6 x nv full Jacobian
                    J_full = np.vstack([jac_pos, jac_rot])  # shape (6, nv)
                    
                    # Test wrench: 5N downward force (no torque)
                    f_ee = np.array([0.0, 0.0, -5.0, 0.0, 0.0, 0.0])  # [fx, fy, fz, tx, ty, tz]
                    
                    # Compute joint torques: τ = J^T f
                    tau = J_full.T @ f_ee  # shape (nv,)
                    
                    # Extract only arm joint torques (first 6)
                    tau_arm = tau[:nv_arm]
                    
                    if self._print_joints:
                        print(f"EE force: {f_ee[:3]} N → Joint torques (arm only): {tau_arm}")

                    # OPTIONAL: Apply these torques instead of position control?
                    # Only if your model uses torque actuators!
                    # self._data.ctrl[:nv_arm] = tau_arm
