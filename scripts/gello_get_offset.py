...
class Args:
    ...
    start_joints: Tuple[float, ...] = (1.57, -1.57, -1.57, -1.57, 1.57, 0)
...
def get_config(args: Args):
    ...
    def get_error(offset: float, index: int, joint_state: np.ndarray) -> float:
        ...
        # print(f"joint_i: {joint_i}, start_i: {start_i}, offset: {offset}", index)
    ...
    #     else:
    #         gripper_offsets = None
    
    # driver.close()
    # return {"joint_offsets": best_offsets,
    #         "gripper_offsets": (np.rad2deg(driver.get_joints()[-1]) - 0.2, 
    #                            np.rad2deg(driver.get_joints()[-1]) - 42),
    #         }
        ...
        if args.gripper:
            ...
            gripper_raw_deg = np.rad2deg(driver.get_joints()[-1])
            gripper_offsets = (gripper_raw_deg - 0.2, gripper_raw_deg - 42)
        else:
            gripper_offsets = None
